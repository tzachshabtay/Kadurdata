...
Use a rolling window of the last 168 hours up to the collection cutoff, not a cutoff at Israeli midnight. Freeze the cutoff once at the start of collection. Include every appearance whose kickoff falls at or after `period.startAt` and before `period.endAt`, provided `api_matches.status` is a completed result (`Ended`, `After ET`, or `After Penalties`). This includes Sunday evening American fixtures that fall on Monday in Israel when they have finished before collection. Query the current foreign-club season and deduplicate imported copies of the same fixture. Apply the completed-match check before deduplication and historical baseline selection as well.
