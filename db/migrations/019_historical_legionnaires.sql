...
as $function$
  ...
  appearance_by_team as (
    select
      appearance.player_id,
      appearance.team_id,
      appearance.season_id,
      coalesce(sum(appearance.minutes_played), 0) as minutes,
      count(distinct appearance.match_id) as appearances
    from season_appearances appearance
    group by appearance.player_id, appearance.team_id, appearance.season_id
  ),
  affiliation_candidates as (
    select
      ...
      coalesce(team_totals.minutes, 0) as team_minutes,
      coalesce(team_totals.appearances, 0) as team_appearances,
    ...
    left join appearance_by_team team_totals
      on team_totals.player_id = stint.player_id
     and team_totals.team_id = stint.team_id
     and team_totals.season_id = stint.season_id
  ...
  affiliation as (
    ...
    order by
      ...
      candidate.team_minutes desc,
      candidate.team_appearances desc,
  ...
  performance as (
    select
      ...
      avg(observation.value_numeric) filter (
        where metric.code = 'rating_365' and observation.value_numeric >= 0
...
as $function$
  ...
  source_resolved as (
    ...
    where observation.subject_type = 'player_match'
      ...
      and observation.value_numeric is not null
      and (metric.value_type <> 'rating' or observation.value_numeric >= 0)
      and (
