...
function validateArticle(article, filename) {
  ...
  if (article.generation?.pipelineVersion !== "match-review-v20") fail("published article was not generated by the current pipeline version");
  ...
  for (const pattern of [/הפך מחריגה לסיפור/, /ההיסטוריה הקצרה שלהם/, /המספרים מספרים/, /צבר(?:ה|ו)? את רוב האיום/, /צבר(?:ה|ו)? את רוב הבעיטות/, /xG\s*;/, /השערים פונו/, /כיתרה\s+\d/, /המצביה/, /ריבוי דו[־-]קרקעי/, /גלים של איומים/, /שימור איזון בנפח/, /נתיב(?:ים|י|י־|ה)?/, /ייצר(?:ה|ו)?[^.]{0,40}בעיטות/, /מצב של המשחק/, /מאזני בעיטות/, /האות הקבוצתי/, /(?:קירב(?:ה|ו)?|התקרב(?:ה|ו)?)\s+(?:את\s+)?(?:ה)?מאזן/, /התקרב(?:ה|ו)?\s+[^.]{0,50}\s+במספר\s+הבעיטות/, /הגיע(?:ה|ו)?\s+טוב\s+יותר\s+לרחבה/, /התקרב(?:ה|ו)?\s+ב?(?:ספירת|מניין)\s+הבעיטות/, /(?:ספירת|מניין)\s+הבעיטות\s+התקרב/, /(?:ה)?בעיטות[^.]{0,60}צמצמ(?:ה|ו)?\s+[^.]{0,30}(?:את\s+)?הפער/, /לפני\s+שמצב\s+המשחק\s+השתנה/, /כל\s+עוד\s+המשחק\s+היה\s+תחרותי/, /שני\s+קצות\s+הרצף/, /סיי(?:ם|מה|מו)\s+את\s+המצב/, /מפת\s+הבעיטות[^.]{0,120}(?:המשחק\s+המלא|אינה\s+מסננת)/]) {
