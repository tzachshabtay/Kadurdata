...
function validateArticle(article, filename) {
  ...
  const expectedPipelineVersion = article.kind === "legionnaire_weekly" ? "legionnaire-weekly-v1" : "match-review-v23";
  ...
  if (article.kind === "legionnaire_weekly") {
    ...
    if ((article.tags ?? []).filter((tag) => tag.kind === "player").length < 2) fail("weekly article must include at least two player tags");
    ...
    if (graphics.length < 2 || graphics.length > 3) fail("weekly article must select 2-3 graphics");
    if (new Set(graphics.map((graphic) => graphic.type)).size !== graphics.length) fail("weekly graphic types must be distinct");
