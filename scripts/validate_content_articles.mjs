...
function validateArticle(article, filename) {
  ...
  if (article.generation?.pipelineVersion !== "match-review-v18") fail("published article was not generated by the current pipeline version");
  ...
  for (const pattern of [/הפך מחריגה לסיפור/, /ההיסטוריה הקצרה שלהם/, /המספרים מספרים/, /צבר(?:ה|ו)? את רוב האיום/, /צבר(?:ה|ו)? את רוב הבעיטות/, /xG\s*;/, /השערים פונו/, /כיתרה\s+\d/, /המצביה/, /ריבוי דו[־-]קרקעי/, /גלים של איומים/, /שימור איזון בנפח/, /נתיב(?:ים|י|י־|ה)?/, /ייצר(?:ה|ו)?[^.]{0,40}בעיטות/, /מצב של המשחק/, /מאזני בעיטות/, /האות הקבוצתי/, /(?:קירב(?:ה|ו)?|התקרב(?:ה|ו)?)\s+(?:את\s+)?(?:ה)?מאזן/, /התקרב(?:ה|ו)?\s+[^.]{0,50}\s+במספר\s+הבעיטות/]) {
