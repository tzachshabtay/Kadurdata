...
const FOOTBALL_HEBREW_GUIDE = [
  ...
  "בגוף הכתבה תאר מיקום של שחקן 'על המגרש'. כאשר מזכירים את כלי ההמחשה עצמו, כתוב 'מפת חום' או 'מפות חום', לעולם לא 'מפת פעילות'.",
  "אל תקצר משפט ערך ל'שוו יחד'. כתוב שהבעיטות היו שוות יחד ערך מסוים או שהסתכמו יחד באותו ערך.",
  "חלוץ מקבל מצבים או הזדמנויות, לא 'סיומים'. סיום הוא הפעולה שהוא מבצע לאחר שקיבל הזדמנות.",
  "אל תחליף ספירת בעיטות בספירת מצבים. אם המקור מודד בעיטות, כתוב בעיטות; כתוב מצבים או הזדמנויות רק כאשר מיקום, איכות או רצף האירוע תומכים בכך.",
  "מסירת מפתח היא פעולה ששחקן מסר או שקבוצה רשמה. אל תכתוב שקבוצה 'קיבלה מסירות מפתח' כאשר המדד מתאר יצירה.",
  "כותרת השוואה לעבר חייבת לומר מה השתנה על המגרש. אל תסתפק בכותרת מופשטת על משחק חריג או שינוי שלא הוכח.",
].join("\n");
...
async function generateAnalysisPlanWithAi(match, evidence, insightCandidates, gameStateContext, analysisFeedback = []) {
  ...
  const response = await postOpenAi({
      ...
      instructions: [
        ...
        "ב-historicalAudit זהה במפורש את האות הזכאי החזק ביותר בכל אחת מקטגוריות הקבוצה והשחקן. אסור להכריז שהביקורת הושלמה אם אות זכאי הושמט בלי בדיקה; מותר ואף רצוי להשאירו מחוץ לכתבה כאשר אינו מסביר את התזה.",
        "כל השוואה אנליטית חייבת להשתמש באותו חלון זמן. אל תחבר נתון של משחק מלא לנתון שנמדד רק עד הרחקה או שער כדי להסיק קשר; אם אין חלון אחיד, סמן את ההבדל במפורש ואל תסיק סיבתיות.",
...
async function reviewEditorialQualityWithAi(match, evidence, analysisPlan, gameStateContext, editorial, attempt) {
  ...
  const response = await postOpenAi({
      ...
      instructions: [
        ...
        "numericClarity ו-evidenceFaithfulness חייבים להיות false כאשר משפט מחליף ספירת בעיטות בספירת מצבים, או מחבר נתון של משחק מלא לנתון עד אירוע בלי לציין את חלונות הזמן.",
        "naturalHebrew ו-footballHebrew חייבים להיות false כאשר הכתבה אומרת שקבוצה 'קיבלה מסירות מפתח' במקום לומר מי מסר, מי יצר או כמה מסירות מפתח הקבוצה רשמה.",
