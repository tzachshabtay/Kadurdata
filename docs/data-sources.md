...
- Transfermarkt's player metadata supplies source-authored Hebrew passport names for Israeli players. Bilingual passport fields are reduced to their Hebrew portion before storage. The general Hebrew-name job also uses exact 365Scores search identities for unmapped players, verified spellings for ambiguous current identities, and English-to-Hebrew transliteration only after the player has been positively identified as Israeli. The transliteration fallback is deliberately not applied to foreign opponents because common names can otherwise be translated as words instead of names.
- A lightweight `hebrew_names` workflow mode reruns resolution for Israeli identities without repeating the full match, foreign-opponent, and legionnaire ingestion. Every run reports unresolved database rows instead of silently accepting gaps.
