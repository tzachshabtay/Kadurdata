...
- A lightweight `hebrew_names` workflow mode reruns name resolution without repeating the full match and legionnaire ingestion. Every run reports unresolved database rows instead of silently accepting gaps.
